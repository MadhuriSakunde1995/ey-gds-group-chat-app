...
from src.utils import convert_utc_to_local

def calculate_hash(sender, timestamp, message, prev_hash=""):
    return hashlib.sha256(
        f"{sender}{timestamp}{message}{prev_hash}".encode()
    ).hexdigest()
...
def handle_new_block(block_json, safe_emit):
    try:
        ...
        if calc_hash == block["hash"]:
            ...
            block_with_display["display_timestamp"] = convert_utc_to_local(
                block["timestamp"]
            )
            safe_emit("receive_message", block_with_display, to_all=True)
            logging.info(
                f"[New Block] Received and broadcasted block from {block['sender']}"
            )
    ...
    try:
        with engine.connect() as conn:
            result = (
                conn.execute(
                    text(
                        "SELECT id, sender, timestamp, message, prev_hash, hash FROM ledger ORDER BY id ASC"
                    )
                )
                .mappings()
                .all()
            ...
            for b in result:
                ...
                if (
                    ...
                    or calculate_hash(
                        b["sender"], timestamp_str, b["message"], b["prev_hash"]
                    )
                    != b["hash"]
                ):
                    return False
