...
from src.config import engine, USER_TIMEZONE
...
def get_last_block_hash():
    try:
        with engine.connect() as conn:
            result = conn.execute(
                text("SELECT TOP 1 hash FROM ledger ORDER BY id DESC")
            )
            row = result.fetchone()
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            for row in result:
                ts = row["timestamp"]
                utc_ts = (
                    ts.strftime("%Y-%m-%d %H:%M:%S") if isinstance(ts, datetime) else ts
                )
                blocks.append(
                    {
                        "sender": row["sender"],
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            """
                    ...
                    {"before_timestamp": before_timestamp, "limit": limit},
                )
                .mappings()
                .all()
            )
